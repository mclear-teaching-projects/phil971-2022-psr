...
\begin{minipage}[t]{2in}
  \flushleft \footnotesize $if(updated)$Updated: \today$endif$\\ \faicon{phone}
  ~$phone$\\ \Faxmachine ~$fax$\\ \faicon{envelope} ~$email$\\ $for(urls)$
  \faicon{globe} ~$urls$ $endfor$\\ \faicon{comment} ~$chat$
\end{minipage}
